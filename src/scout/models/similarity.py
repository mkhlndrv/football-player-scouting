...
def profile(player_match: pd.DataFrame, shots: pd.DataFrame) -> pd.DataFrame:
...
def neighbours(profiles: pd.DataFrame, k: int = K) -> pd.DataFrame:
    ...
    for (role, season), group in profiles.groupby(["role", "season"]):
        if len(group) <= k:
            continue
        model = NearestNeighbors(n_neighbors=k + 1).fit(group[FEATURES].to_numpy(float))
        dist, idx = model.kneighbors(group[FEATURES].to_numpy(float))
